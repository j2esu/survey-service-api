...
class Repo {
    ...
    fun deleteSurvey(user: User, surveyId: String): Boolean {
        val survey = getSurvey(surveyId) ?: return false
        require(user.id == survey.ownerId) { "User doesn't own this survey" }
        return surveys.removeIf { it.id == surveyId }
    }
